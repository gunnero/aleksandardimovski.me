...
final class InitializeCandidateProfile extends Command
{
    ...
    public function handle(): int
    {
        ...
        $states = [];
        foreach (['full_name', 'professional_email', 'phone', 'location', 'portfolio_url', 'github_url', 'linkedin_url', 'primary_title', 'professional_summary', 'education_json', 'languages_json', 'verified_skills_json', 'employment_history_json'] as $key) {
            $states[$key] = 'verified';
        ...
        $profile->fill(['full_name' => config('portfolio.name'), 'professional_email' => config('portfolio.email'), 'phone' => config('portfolio.phone'), 'location' => config('portfolio.location'), 'portfolio_url' => rtrim(config('app.url'), '/'), 'github_url' => config('portfolio.github'), 'linkedin_url' => config('portfolio.linkedin_url'), 'primary_title' => 'Senior PHP / Laravel Engineer and Backend & Product Engineer', 'professional_summary' => config('resume.summary'), 'education_json' => [config('resume.education')], 'languages_json' => config('resume.languages'), 'verified_skills_json' => config('resume.skill_groups'), 'employment_history_json' => config('resume.experience'), 'field_states_json' => $states]);
