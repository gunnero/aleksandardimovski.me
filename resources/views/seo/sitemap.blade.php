...
@foreach([route('home'),route('about'),route('projects.index'),route('experience'),route('resume'),route('articles.index'),route('contact')] as $url)<url><loc>{{ $url }}</loc></url>@endforeach
