    ...
    <div class="review-grid">
        ...
        <aside class="review-actions">
            ...
            @if($application->opportunity->review_status === 'approved_for_preparation')
                <section class="workspace-card"><h2>Reject opportunity</h2><p>Rejecting withdraws this application and permanently blocks submission approval. Prepared documents remain private and are not deleted.</p><form method="post" action="{{ route('workspace.applications.decision',$application) }}" data-rejection-rule>@csrf<input type="hidden" name="action" value="reject"><div class="form-field"><label for="rejection-reason">Rejection reason</label><select id="rejection-reason" name="rejection_reason" required><option value="">Select a reason</option><option value="remote_policy_mismatch">Remote policy does not match requirements</option><option value="compensation">Compensation mismatch</option><option value="location">Location or timezone mismatch</option><option value="weak_fit">Role no longer suitable</option><option value="company_concern">Company concern</option><option value="other">Other</option></select></div><div class="form-field"><label for="rejection-note">Rejection note <span>Optional</span></label><textarea id="rejection-note" name="note" rows="4" placeholder="Remote work is limited to 30 days per year, which does not match my fully remote requirement."></textarea></div>
                <label class="check-field"><input type="checkbox" name="use_as_rule" value="1" data-use-rule><span>Use this rejection as a rule for future jobs</span></label><div data-rule-fields hidden><div class="form-field"><label for="rule-severity">Rule severity</label><select id="rule-severity" name="rule_severity" data-rule-severity><option value="job_only">This job only</option><option value="hard_exclusion">Hard exclusion</option><option value="strong_penalty">Strong penalty</option><option value="soft_penalty">Soft penalty</option></select></div><div class="form-field"><label for="rule-scope">Rule scope</label><select id="rule-scope" name="rule_scope"><option value="all_jobs">All jobs</option><option value="role_family">Similar roles</option><option value="company">This company</option><option value="source">This source</option></select></div><div class="callout callout--warning"><strong>Generated rule preview</strong><p data-rule-preview>No reusable rule will be created.</p></div><label class="check-field" data-hard-confirm hidden><input type="checkbox" name="confirm_hard_rule" value="1"><span>I explicitly confirm this reusable hard exclusion.</span></label></div>
                <button class="button button--danger button--full" type="submit">Reject and close application</button></form></section>
            @endif
        </aside>
    </div>
</x-workspace.layout>
