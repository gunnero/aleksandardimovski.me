...
<section class="section"><div class="container timeline"><div class="notice employment-note"><strong>Concurrent working arrangements</strong><p>{{ config('resume.employment_context') }}</p></div>
@foreach(config('resume.experience') as $role)
<article><p class="eyebrow">{{ $role['period'] }}</p><h2>{{ $role['title'] }}</h2><h3>{{ $role['company'] }}</h3>@if($role['arrangement'])<p class="role-arrangement">{{ $role['arrangement'] }}</p>@endif<p>{{ $role['summary'] }}</p>@if($role['highlights'])<ul>@foreach($role['highlights'] as $item)<li>{{ $item }}</li>@endforeach</ul>@endif</article>
@endforeach
